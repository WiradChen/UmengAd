# UMAD SDK — 带本地智能优化的广告聚合引擎

> **唯一一个不用接后台、自动越赚越多的 Android 广告聚合 SDK**

一站式接入穿山甲、快手、优量汇、百度、瑞狮等 20+ 主流广告平台。内置 **本地智能收益引擎**，无需上传数据、无需后台配置，接入后自动学习优化，收益越用越高。

## 为什么选 UMAD？

| 特性 | UMAD | 传统聚合 SDK |
|:---|:---:|:---:|
| 智能优化 | ✅ 本地自动优化，开箱即用 | ❌ 需手动配分层 / 接后台 |
| 数据隐私 | ✅ 纯本地计算，不上传任何数据 | ❌ 广告数据全量上传服务商 |
| 接入成本 | ✅ 一个 AAR + JSON 配置，10 分钟搞定 | ❌ 注册后台、创建应用、配置广告位 |
| 优化效果 | ✅ 自动动态排序 + 底价过滤 + 延迟加载 | ❌ 静态瀑布流，需人工定期调整 |
| 使用费用 | ✅ 完全免费 | ❌ 高级优化功能需付费 |
...
内置 `AdOptimizer` 模块，基于本地历史数据自动优化瀑布流，**越用收益越高**：

- **动态排序**：根据填充率 × eCPM × 加载速度 × **点击率** 自动重排，高价且高点击的广告优先展示
- **底价过滤**：预估打不过当前最高价的平台直接跳过，省流量、保护账号
- **动态延迟加载**：第一个广告返回后多等一小会儿，确保展示的是最高价
- **新平台保护**：新接入平台给足探索机会，不会被老平台压制
...
一次集成，对接穿山甲、快手、优量汇、百度、瑞狮等多家主流广告平台，省去逐个对接的成本。

### ⚖️ Waterfall + Bidding 双模式

支持 **瀑布流** 和 **客户端竞价** 两种模式，可混合使用：高层 Bidding 比价，底层 Waterfall 兜底，兼顾收益与填充。

### 🎯 六大广告类型全覆盖

开屏、横幅、插屏、激励视频、全屏视频、信息流，所有主流广告场景全部支持。

### ⚙️ 配置灵活

JSON 配置广告位，无需硬编码，支持静态配置 + 动态下发两种模式。

### 🪶 轻量稳定

API 简洁，接入成本低，核心代码经过线上大规模验证，运行稳定。

---

## 核心机制

### 🧠 一、本地智能收益引擎（AdOptimizer）

这是 UMAD 最核心的差异化能力。SDK 自动统计每个广告位、每个平台的历史表现数据，在本地实时优化瀑布流策略。

#### 1. 动态排序

根据各平台的 **填充率 × eCPM × 加载速度 × 点击率(CTR)** 计算综合得分，得分高的排前面。

- 点击率学习：自动统计每个平台的点击率，高点击平台加权排前（让"高 eCPM + 高点击"的广告优先展示）
- 冷启动保护：展示 <10 次 或 点击 <3 次 时，点击率因子取中性值 1.0，不扰动新平台
